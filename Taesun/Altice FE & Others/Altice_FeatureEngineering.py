...
Title: Create Custom Transformers to Primarily Conduct Feature Engineering
...
1. Pre-Processing:
    ...
    8. `Remove_IrrelevantCollinearFeatures`
        - Remove features with low correlation with target (e.g. abs(Corr) <= 0.005).
        - Remove features with high correlation with other features (e.g. abs(Corr) >= 0.95).
...
2. Feature Engineering for Numerical Features
    ...
    2. numpy-Based Transformers:
        ...
        - 
    3. Model-Based Transformers:
        - `DecisionTreeDiscretizer`
        -
    4. Utility Transformers/Functions:
...
class Remove_IrrelevantCollinearFeatures(BaseEstimator, TransformerMixin):
    '''
    Purpose
    ---------------
    A custom transformer
    - (1) to remove features with low correlation with target and 
    - (2) to remove features with high correlation with other features
    ...
    y: a pandas series that represent a churn status
       - default: y=Given
    corr_threshold_target: abs(correlation) ~ [0, 1]
       - default: corr_threshold_target=0.005 
       - minimum correlation with target
    corr_threshold_features: abs(correlation) ~ [0, 1]
       - default: corr_threshold_features=0.90 
       - maximum correlation among features
    ...
    datafrme
        ...
        - summary_dropped_: a df with pairwise correlation between dropped/correlated features
    list
        - features_dropped_Irrelevant_ : features that drop due to low correlation with target
        - features_dropped_Collinear_  : features that drop due to high correlation with other features
        - features_kept_: features that are kept
    ...
    def __init__(self, corr_threshold_target=0.005, corr_threshold_features=0.90):
        self.corr_threshold_target   = corr_threshold_target
        self.corr_threshold_features = corr_threshold_features        

        if (self.corr_threshold_target<0) | (self.corr_threshold_target>1):
            raise ValueError('Correlation threshold with target should be in the range of 0 to 1.')
        if (self.corr_threshold_features<0) | (self.corr_threshold_features>1):
            raise ValueError('Correlation threshold among features should be in the range of 0 to 1.')

        print("'Remove_IrrelevantCollinearFeatures' requires target y.")            
    ...
    def fit(self, X, y):
        ...
        features_relevant.sort()
        tmp_features_NUM  = X[features_relevant].select_dtypes(exclude=[object, 'category']).columns.tolist()
        features_CAT      = X[features_relevant].select_dtypes(include=[object, 'category']).columns.tolist()

        # Include NUMERICAL features with abs(corr with target) >= corr_threshold_target
        tmp_corr          = X[tmp_features_NUM].apply(lambda x: x.corr(y))
        features_NUM      = tmp_corr[abs(tmp_corr) >= self.corr_threshold_target].index.tolist()

        # Features dropped due to low correlation with sales
        features_dropped_Irrelevant_ = list(set(tmp_features_NUM) - set(features_NUM))
        features_dropped_Irrelevant_.sort()
        self.features_dropped_Irrelevant_ = features_dropped_Irrelevant_

        # Sort Features in abs(correlation) with Target.
        corr_all    = pd.concat([y, X[features_NUM]], axis=1).corr()
        corr_target = corr_all.iloc[1:, 0].to_frame()
        corr_target.columns = ['corr_target']
        features_NUM  = corr_all.iloc[1:, 0].abs().sort_values(ascending=False).index.tolist()
        ...
        # 2. Identify Features with abs(correlation) > corr_threshold_features
        features_dropped_Collinear_ = [fe for fe in corr_upper.columns \
                                       if any(corr_upper[fe].abs() > self.corr_threshold_features)]
        features_dropped_Collinear_.sort()                         
        self.features_dropped_Collinear_ = features_dropped_Collinear_
        # Combine not dropping features with CAT features
        self.features_kept_    = list(set(features_NUM) - set(self.features_dropped_Collinear_)) + features_CAT
        
        # 3. Create Pairwise Correlation Dataframe
        summary_dropped_ = pd.DataFrame(columns = ['drop_feature', 'corr_feature', 'pairwise_corr'])
        
        for fe in self.features_dropped_Collinear_:
            corr_features = list(corr_upper.index[corr_upper[fe].abs() > self.corr_threshold_features])
            corr_values   = list(corr_upper[fe][corr_upper[fe].abs() > self.corr_threshold_features])
            drop_features = [fe for _ in range(len(corr_features))]    
            temp_df       = pd.DataFrame.from_dict({'drop_feature': drop_features,
                                                    'corr_feature': corr_features,
                                                    'pairwise_corr': corr_values})
        ...
        print('\n' + '*'*100 )
        print(f'Total NUMERICAL Features: {len(tmp_features_NUM)} features examined')  
        print(f'Irrelevant Features     : {len(self.features_dropped_Irrelevant_)} features dropped due to abs(correlation ) <= {self.corr_threshold_target} with target')        
        print(f'Collinear Features      : {len(self.features_dropped_Collinear_)} features dropped due to abs(correlation ) > {self.corr_threshold_features} with other features')
        print('*'*100)
...
class Use_DefaultDataType(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        features_relevant.sort()
        features_NUM = X_tmp[features_relevant].select_dtypes(exclude=[object, 'category']).columns.tolist()
        features_CAT = X_tmp[features_relevant].select_dtypes(include=[object, 'category']).columns.tolist()
...
class DecisionTreeDiscretizer(BaseEstimator, TransformerMixin):
    ...
    Purpose
    ---------------
    A custom transformer to discretize NUMERICAL features using Decision Tree
    ...
    y: a pandas series that represent a churn status
       - default: y=Given
    cv: the number of cross-validation to determine the optimal depth of tree
       - default: cv=3
    max_tree_depth: the maximum depth of tree to be tested
        - default: max_tree_depth = 6
    min_node_size: the minimum of number of samples in the final nodes
        - default: min_node_size = 0.005
        - min_node_size = 0.005 --> minimum 0.5% of the total sample
        - e.g. (1) sample size = 10k & (2) min_node_size = 0.005 
        -      --> 10k * 0.005 = at least 500 samples in the final nodes
    corr_threshold_target: abs(correlation) ~ [0, 1]
       - default: corr_threshold_target=0.005 
       - minimum correlation with target
    prefix: a string to add a prefix to all column names
    ...
    feature_mapper_: 
        - a data dictionary of mapping features to corresponding decision trees

     References
    ---------------        
    Feature Engine: 
        - https://github.com/solegalli/feature_engine/blob/master/feature_engine/discretisers.py
    ...
    def __init__(self, cv=3, corr_threshold_target=0.005, max_tree_depth=6, min_node_size=0.005, prefix=None, suffix=None):
        self.cv                      = cv
        self.corr_threshold_target   = corr_threshold_target
        ...
        if (self.corr_threshold_target<0) | (self.corr_threshold_target>1):
            raise ValueError("'corr_threshold_target' should be in the range of 0 to 1.")
        if (self.max_tree_depth<1) | (self.max_tree_depth>10):
            raise ValueError("'max_tree_depth' should be in the range of 1 to 10.")
        if (self.corr_threshold_target<0) | (self.corr_threshold_target>1):
            raise ValueError("'min_node_size' should be in the range of 0 to 1.")

        print("'DecisionTreeDiscretizer' requires target y.")            
    ...
    def fit(self, X, y):
        ...
        # Include NUMERICAL features with abs(corr with target) >= corr_threshold_target
        tmp_corr          = X[tmp_features_NUM].apply(lambda x: x.corr(y))
        features_NUM_     = tmp_corr[abs(tmp_corr) >= self.corr_threshold_target].index.tolist()
        features_NUM_.sort()

        #1. Run Decision Tree by Feature
        # Create Feature Mapping Dictionary
        feature_mapper_    = {}

        for fe in features_NUM_:
            auc_scores     = []
            max_count      = self.max_tree_depth + 1

            for i in np.arange(1, max_count): # Evaluate the optimal depth from 1 up to self.max_tree_depth
                clf        = DecisionTreeClassifier(max_depth=i, min_samples_leaf=self.min_node_size)
                tmp_scores = cross_val_score(clf, X[fe].to_frame(), y, cv=self.cv, scoring='roc_auc')
                auc_scores.append(np.mean(tmp_scores))    

            optimal_depth       = np.arange(1, max_count)[np.argmax(auc_scores)]
            clf                 = DecisionTreeClassifier(max_depth=optimal_depth, min_samples_leaf=self.min_node_size)
            clf.fit(X[fe].to_frame(), y)
            feature_mapper_[fe] = clf

        self.features_NUM_      = features_NUM_
        self.feature_mapper_    = feature_mapper_
    ...
    def transform(self, X, y=None):
        # Note: A custom utility function, 'get_feature_name' is used.        
        tmp_df         = X[self.features_NUM_]

        # Now feature values are replaced by predicted probabilities made by decision trees.
        for fe in self.features_NUM_:
            tmp_df[fe] = self.feature_mapper_[fe].predict_proba(tmp_df[fe].to_frame())[:, 1]
...
class FeatureMaker(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        tmp_df['grp_payment_method'] = tmp_df.idxmax(axis=1).str.replace('_payments_6m', '').\
                                       str.upper()
        tmp_transformed = pd.concat([tmp_transformed, tmp_df[['grp_payment_method']]], axis=1)                               
        ...
        tmp_transformed = pd.concat([tmp_transformed, tmp_df[['grp_payment_25dollar', 'grp_payment_10dollar']]], axis=1)                               
        ...
        tmp_df['grp_payment_change_5pct'] = pd.cut(tmp_df.ttl_rev_perc_change_6m, 
                                                   bins=tmp_bins, labels=tmp_labels)        
        tmp_df          = tmp_df[['grp_payment_change_5dollar', 'grp_payment_change_10dollar', \
                                'grp_payment_change_2pct', 'grp_payment_change_5pct']]
        tmp_transformed = pd.concat([tmp_transformed, tmp_df], axis=1)                               
        ...
        tmp_df['grp_payment_income'] = pd.cut(tmp_df.ratio_payment_income, \
                                              bins=tmp_bins, labels=tmp_labels)
        tmp_transformed              = pd.concat([tmp_transformed, tmp_df[['ratio_payment_income', 'grp_payment_income']]], axis=1)                               
        ...
        tmp_transformed          = pd.concat([tmp_transformed, tmp_df[['grp_call_csc']]], axis=1)                               
        ...
        tmp_transformed           = pd.concat([tmp_transformed, tmp_df[['grp_call_bill']]], axis=1)                               
        ...
        tmp_df['grp_call_csr']   = pd.cut(tmp_df.call_csr_total, \
                                          bins=tmp_bins, labels=tmp_labels)        
        tmp_transformed          = pd.concat([tmp_transformed, tmp_df[['grp_call_csr']]], axis=1)                               
        ...
        tmp_df['grp_call_tsr']   = pd.cut(tmp_df.call_tsr_total, \
                                        bins=tmp_bins, labels=tmp_labels)
        tmp_transformed          = pd.concat([tmp_transformed, tmp_df[['grp_call_tsr']]], axis=1)                               
